...
double computeLogNormConst(Vector_nD &k, double lower, double upper) {
     ...
     if ((sqrt(k1*k1 + k2*k2 - 2*k1*k3) + k2) < exactLimit) {
          ...
          double resExact = integrate_quad(&logNormConstExactIntegrand, extraArguments, lower, upper);
          return (log(resExact));
     } else {
          double scaleTerm = sqrt(k1*k1 + k3*k3 - 2*k1*k3) + k2;
          double extraArguments[4] = {k1, k2, k3, scaleTerm};
          double resApproximate = integrate_quad(&logNormConstApproximateIntegrand, extraArguments, lower, upper);
